...
window.dtx["%1$s"] = function(opts) {
    ...
    @if(isset($editors))
        @foreach($editors as $editor)
            var {{$editor->instance}} = window.{{ config()->get('datatables-html.namespace', 'LaravelDataTables') }}["%1$s-{{$editor->instance}}"] = new $.fn.dataTable.Editor({!! $editor->toJson() !!});
            {!! $editor->scripts  !!}
        ...
        @endforeach
    @endif
    return window.{{ config()->get('datatables-html.namespace', 'LaravelDataTables') }}["%1$s"] = $("#%1$s").DataTable($.extend(%2$s, opts));
}
